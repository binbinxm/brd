...
def main():
    ...
    argparser.add_argument('offset', type=str,
                           help='internal register offset of the eeprom, hex format')
    argparser.add_argument('data', type=str, nargs='?', default=None,
                           help='write value, hex format, if this field is not empty, then I assume it\'s a write op')
...
def read_one_byte(url, addr, offset):
    ...
    try:
        ...
        #tmp = slave.read(tmp, start=True, relax=True)
        value = int.from_bytes(tmp, 'big') & 0xff
...
def write_one_byte(url, addr, offset, data):
    ...
    try:
        i2c.configure(url)
        slave = i2c.get_port(addr)
        string = offset.to_bytes(1,'big') + b'\x01' + data.to_bytes(1,'big')
